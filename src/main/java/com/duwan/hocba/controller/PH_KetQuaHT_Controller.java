...
public class PH_KetQuaHT_Controller {
	...
	@GetMapping("/phuhuynh/ketquahoctap/{hocsinh_id}/{lop}")
	public String showPH_KQHT2(HttpSession session, Model model, @PathVariable int hocsinh_id, @PathVariable int lop) {
		PhuHuynhObject ph = (PhuHuynhObject) session.getAttribute("phuhuynh");
	    if (ph != null) {	  
	        model.addAttribute("list_lop", getList(5));
